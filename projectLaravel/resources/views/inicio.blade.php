...
<div class="p-4 lg:p-6 space-y-4 lg:space-y-6 flex-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <!-- Card 1 -->
        <div onclick="showSummaryAlert()" data-slot="card" class="bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 flex flex-col gap-3 rounded-lg relative overflow-hidden border border-slate-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all cursor-pointer p-4 group">
            ...
            <div class="flex items-center justify-between z-10">
                ...
                <div class="flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 badge-dark-emerald px-2 py-0.5 rounded-full border border-emerald-100 dark:border-emerald-800">
                    ...
                    {{ $totalBienes > 0 ? number_format(($bienesOperativos / $totalBienes) * 100, 0) : 0 }}%
                </div>
            </div>
            <div>
                <p class="text-2xl font-bold text-slate-900 dark:text-gray-100 leading-tight">{{ number_format($bienesOperativos) }}</p>
                <p class="text-xs text-slate-500 dark:text-gray-400 font-medium mt-0.5">Bienes Operativos</p>
            </div>
        </div>
        ...
        <div data-slot="card" class="bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 flex flex-col gap-3 rounded-lg relative overflow-hidden border border-slate-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all p-4 group">
            ...
            <div class="flex items-center justify-between z-10">
                ...
                <div class="flex items-center gap-1 text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-50 badge-dark-amber px-2 py-0.5 rounded-full border border-amber-100 dark:border-amber-800">
                    ...
                    {{ $bienesEnReparacion ?? 0 }} Pend.
                </div>
            </div>
            <div>
                <p class="text-2xl font-bold text-slate-900 dark:text-gray-100 leading-tight">{{ number_format($bienesEnReparacion) }}</p>
                <p class="text-xs text-slate-500 dark:text-gray-400 font-medium mt-0.5">Bienes En Mantenimiento</p>
            </div>
        </div>
        ...
        <div data-slot="card" class="bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 flex flex-col gap-3 rounded-lg relative overflow-hidden border border-slate-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all p-4 group">
            ...
            <div class="flex items-center justify-between z-10">
                ...
                <div class="flex items-center gap-1 text-xs font-medium text-red-600 dark:text-red-400 bg-red-50 badge-dark-red px-2 py-0.5 rounded-full border border-red-100 dark:border-red-800">
                    {{ $bienesDanados ?? 0 }} Fallas
                </div>
            </div>
            <div>
                <p class="text-2xl font-bold text-slate-900 dark:text-gray-100 leading-tight">{{ number_format($bienesDanados) }}</p>
                <p class="text-xs text-slate-500 dark:text-gray-400 font-medium mt-0.5">Bienes Fuera de Servicio</p>
            </div>
        </div>

        <!-- Card 4 -->
        <div data-slot="card" class="bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 flex flex-col gap-3 rounded-lg relative overflow-hidden border border-slate-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all p-4 group">
            <div class="absolute top-0 right-0 w-16 h-16 bg-slate-50 dark:bg-slate-800/50 rounded-full -mr-6 -mt-6 opacity-50 group-hover:scale-110 transition-transform"></div>
            <div class="flex items-center justify-between z-10">
                <div class="p-2 bg-slate-100 dark:bg-slate-800 rounded-md text-slate-600 dark:text-slate-400">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-900 dark:text-gray-100 leading-tight">{{ number_format($bienesDesincorporados) }}</p>
                <p class="text-xs text-slate-500 dark:text-gray-400 font-medium mt-0.5">Bienes Desincorporados</p>
            </div>
        </div>
    </div>
    ...
    <div class="mt-4 grid grid-cols-1 xl:grid-cols-2 gap-6">
        <!-- AC Section -->
        <div>
            <h4 class="text-sm font-bold text-slate-700 dark:text-gray-200 uppercase tracking-wide mb-3 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <!-- AC Stat 1 -->
                <div class="bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase">Total Equipos</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $acTotal ?? 0 }}</p>
                    </div>
                    <div class="h-8 w-8 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-400">
                ...
                <!-- AC Stat 2 -->
                <div class="bg-emerald-50/50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-emerald-600 dark:text-emerald-400 font-semibold uppercase">Operativos</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $acOperativos ?? 0 }}</p>
                    </div>
                ...
                <!-- AC Stat 3 -->
                <div class="bg-red-50/50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-red-600 dark:text-red-400 font-semibold uppercase">Criticos/Falla</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $acCriticos ?? 0 }}</p>
                    </div>
                    <div class="h-8 w-8 rounded-full bg-red-100 dark:bg-red-900/50 flex items-center justify-center text-red-600 dark:text-red-400">
        ...
        <div>
            <h4 class="text-sm font-bold text-slate-700 dark:text-gray-200 uppercase tracking-wide mb-3 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-layout-dashboard text-slate-500 dark:text-gray-400">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <!-- Total Bienes -->
                <div class="bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold uppercase">Total Bienes</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $totalBienes ?? 0 }}</p>
                    </div>
                ...
                <!-- Total Categorías -->
                <div class="bg-pink-50/50 dark:bg-pink-900/20 border border-pink-100 dark:border-pink-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-pink-600 dark:text-pink-400 font-semibold uppercase">Categorías</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $totalCategorias ?? 0 }}</p>
                    </div>
                    <div class="h-8 w-8 rounded-full bg-pink-100 dark:bg-pink-900/50 flex items-center justify-center text-pink-600 dark:text-pink-400">
                ...
                <!-- Pendientes -->
                <div class="bg-amber-50/50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-lg p-3 flex items-center justify-between">
                    <div>
                        <p class="text-xs text-amber-600 dark:text-amber-400 font-semibold uppercase">Mant. Pendientes</p>
                        <p class="text-xl font-bold text-slate-800 dark:text-gray-100">{{ $totalPendientes ?? 0 }}</p>
                    </div>
...
<span id="recharts_measurement_span" aria-hidden="true" style="position: absolute; top: -20000px; left: 0px; padding: 0px; margin: 0px; border: none; white-space: pre; font-size: 12px; letter-spacing: normal;">65</span>
    ...
    @push('scripts')
        ...
        function drawCurveChart() {
            ...
            var textColor = isDark ? '#cbd5e1' : '#000000';
            var titleColor = isDark ? '#f1f5f9' : '#000000';

            var options = {
                title: 'Últimos 6 Meses (Incluyendo Mes Actual)',
                curveType: 'function',
                legend: { 
                    position: 'bottom',
                ...
                hAxis: {
                    ...
                    gridlines: { color: isDark ? '#334155' : '#e5e7eb' }
                },
                vAxis: {
        ...
        function drawPieChart() {
            ...
            // Detectar si está en modo oscuro
            var isDark = document.documentElement.classList.contains('dark');
            var textColor = isDark ? '#cbd5e1' : '#222222'; // slate-300 para dark
            var titleColor = isDark ? '#f1f5f9' : '#000000'; // slate-100 para dark
            ...
            var options = {
                ...
                pieSliceTextStyle: {
                    color: '#ffffff' // Texto dentro de las rebanadas siempre blanco
                }
            };
