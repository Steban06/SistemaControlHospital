...
<div id="historyBienModal" class="fixed inset-0 z-[99999] hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="z-index: 99999;">
    <!-- Overlay Background -->
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity duration-300 ease-out z-[99998] opacity-0" onclick="closeHistoryModal()" style="z-index: 99998; background-color: rgba(0, 0, 0, 0.6); backdrop-filter: blur(4px);"></div>

    <!-- Modal Content -->
    <div role="dialog" id="historyBienContent" 
         class="bg-white fixed top-[50%] left-[50%] z-[100000] flex flex-col translate-x-[-50%] translate-y-[-50%] rounded-xl border shadow-2xl duration-300 ease-out sm:max-w-lg max-w-5xl w-[95vw] max-h-[90vh] opacity-0 scale-95 transition-all" 
         tabindex="-1" style="pointer-events: auto; z-index: 100000;">
         
        <!-- Close Button (X) -->
        <button type="button" onclick="closeHistoryModal()" class="cursor-pointer absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none p-1 hover:bg-gray-100 hover:text-red-500 z-50 btn-color-red">
        ...
        <div class="flex-shrink-0 px-6 pt-6 pb-4 border-b border-gray-200">
            <div class="flex flex-col gap-2 text-center sm:text-left">
                ...
                <p id="history_bien_subtitle" class="text-sm text-gray-500">
                    <!-- Populated by JS -->
                        ...
                        <div class="grid grid-cols-[1fr_auto] gap-2">
                            <div class="flex items-start gap-3">
                                <div class="bg-blue-100 p-2 rounded-full flex-shrink-0">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-blue-600"><path d="M12 20v-6M6 20V10M18 20V4"/></svg>
                                </div>
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 font-medium text-xs bg-blue-100 text-blue-700 border-blue-300">Asignación</span>
                                        <span class="text-xs text-gray-500">25 de Enero de 2026</span>
                                    </div>
                                    <h4 class="font-semibold text-sm">Asignación inicial de activo</h4>
                                    <p class="text-xs text-gray-600 mt-1">Registrado por: Admin</p>
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex-shrink-0 px-6 py-4 rounded-xl border-t border-gray-200 bg-gray-50">
             <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                <div class="flex items-center justify-between w-full">
                    <p class="text-xs text-gray-500">1 registro(s) encontrado(s)</p>
                    <button type="button" onclick="closeHistoryModal()" class="btn-color-red inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all bg-white text-gray-700 hover:bg-gray-50 border border-gray-300 shadow-sm h-9 px-4 py-2 text-sm">
                        Cerrar
                    </button>
                </div>
            </div>
